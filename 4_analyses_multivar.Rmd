...
fig2<-
  # All years
  ggplot(data_figure2,aes(x=value))+
    geom_histogram(color="black",size=0.1,fill="grey",position="identity")+
    facet_grid2(c("variable"),scales="free",independent="x")+
    xlab(NULL)+ylab("Count")+my_theme()+
    theme(strip.text.y = element_blank())+
  # Each year
  ggplot(data_figure2,aes(x=value,fill=year))+
    geom_histogram(color="black",size=0.1,position="identity")+
    scale_fill_manual(values=c("#E69F00","#56B4E9","#009E73"))+
    facet_grid2(c("variable","year"),scales="free",independent="x")+
    xlab(NULL)+ylab(NULL)+my_theme()+
  plot_layout(ncol=2,widths=c(0.25,0.75))+
  plot_annotation(caption = "Value of the moment") &
  theme(plot.caption = element_text(hjust = 0.50,family="serif",size=16))
ggsave(fig2,filename="output/figures/fig2.tiff",
       device="tiff",width=20,height=18,units="cm",dpi=300,compression="lzw")
```
...
## Figure 2: Effects on relative iftness
...
## Figure 3: Effects on fruit initiation, fruit maturation and proportion of seeds escaping predation

```{r}
data_figure3<-rbind(prediction_fr_init,
      prediction_fr_mat,
...
#   guides(linetype=F)
# fig3
# ggsave(file="output/figures/fig3.tiff", plot=fig3, width=10, height=12)
# ggsave(file="output/figures/fig3.svg", plot=fig3, width=10, height=12)
```

```{r}
legend_fig3<-get_legend(ggplot()+
  # geom_point(data=data_ids%>%
  #              dplyr::select(year,fr_init,avFD_std,var_std,skew_std,kurt_std)%>%
  #              pivot_longer(cols=avFD_std:kurt_std,
  #                           names_to="predictor",values_to="value_predictor")%>%
...
fig3<-grid.arrange(
  ...
  ggplot()+
    facet_grid2(c("predictor","year"), 
                labeller=labeller(predictor=predictor.labs), 
                scales = "fixed")+
    geom_ribbon(data=bind_rows(data_figure3%>%
                                 filter(predictor=="kurt_std"),
                               new_row),
                aes(x=x,y=predicted,ymin=conf.low,ymax=conf.high,fill=response),
                alpha=0.3)+
    geom_line(data=bind_rows(data_figure3%>%
                               filter(predictor=="kurt_std"),
                             new_row),
              aes(x=x,y=predicted,linetype=type,color=response),
              linewidth=1)+
...
## Figure 3 with space to add coefficient plots (not used)

```{r}
plot_87<-ggplot()+
    facet_grid2(c("predictor"),labeller=labeller(predictor=predictor.labs), 
                scales = "free", independent = "x")+
    geom_ribbon(data=data_figure3%>%filter(year==1987),
                aes(x=x,y=predicted,ymin=conf.low,ymax=conf.high,fill=response),
                alpha=0.3)+
    geom_line(data=data_figure3%>%filter(year==1987),
...
plot_88<-ggplot()+
    facet_grid2(c("predictor"),labeller=labeller(predictor=predictor.labs), 
                scales = "free", independent = "x")+
    geom_ribbon(data=data_figure3%>%filter(year==1988),
                aes(x=x,y=predicted,ymin=conf.low,ymax=conf.high,fill=response),
                alpha=0.3)+
    geom_line(data=data_figure3%>%filter(year==1988),
              aes(x=x,y=predicted,linetype=type,color=response),linewidth=1)+
    scale_linetype_manual(values=c("solid","dotted"))+
    scale_y_continuous(limits=c(0,1))+  ylab("Proportion")+
    xlab("Value of the moment")+my_theme()+
...
plot_89<-ggplot()+
    facet_grid2(c("predictor"),labeller=labeller(predictor=predictor.labs), 
                scales = "free", independent = "x")+
    geom_ribbon(data=data_figure3%>%filter(year==1989),
                aes(x=x,y=predicted,ymin=conf.low,ymax=conf.high,fill=response),
                alpha=0.3)+
    geom_line(data=data_figure3%>%filter(year==1989),
              aes(x=x,y=predicted,linetype=type,color=response),linewidth=1)+
    scale_linetype_manual(values=c("solid","dotted"))+
    scale_y_continuous(limits=c(0,1))+  ylab("Proportion")+
    xlab("Value of the moment")+my_theme()+
    ...
    scale_color_manual(values=c("#E69F00","#009E73"),name=NULL,
                       labels = c("Fruit initiation",
                                  "Seeds escaping predation"))+
    guides(linetype=F)+theme(plot.margin=unit(c(0.5,0.5,0.5,6),"cm"))
fig3_space<-grid.arrange(plot_87,plot_88,plot_89,ncol=3)
fig3_space
ggsave(file="output/figures/fig3_space.tiff", plot=fig3_space, width=18, height=12)
ggsave(file="output/figures/fig3_space.svg", plot=fig3_space, width=18, height=12)
```
